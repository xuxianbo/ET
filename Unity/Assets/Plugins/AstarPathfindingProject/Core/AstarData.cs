...
namespace Pathfinding {
	...
	public class AstarData {
		...
		/// </summary>
		static System.Type[] AllGraphTypes;
		public void FindGraphTypes () 
		{
			if (AllGraphTypes != null)
			{
				graphTypes = AllGraphTypes;
				return;
			}
#if !ASTAR_FAST_NO_EXCEPTIONS && !UNITY_WINRT && !UNITY_WEBGL
			var graphList = new List<System.Type>();
			var valueType = typeof(System.ValueType);
			var enumType = typeof(System.Enum);
			var navGraphType = typeof(NavGraph);
...
#endif
					if (baseType == valueType) continue;
					if (baseType == enumType) continue;

					if(navGraphType.IsAssignableFrom(type))
						graphList.Add(type);
				}
			}
