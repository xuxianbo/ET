...
namespace Pathfinding.Serialization {
	...
	public class GraphSerializationContext {
		...
		private MemoryStream memorySteam;
		...
		public GraphSerializationContext (BinaryReader reader, GraphNode[] id2NodeMapping, uint graphIndex, GraphMeta meta) {
			...
			this.meta = meta;
			if(reader != null && BitConverter.IsLittleEndian) memorySteam = reader.BaseStream as MemoryStream;
		}
	...
	public class AstarSerializer {
		...
		public bool OpenDeserialize (byte[] bytes) {
			...
			zipStream = new MemoryStream(bytes);
			try {
#if NETFX_CORE
...
#else
			//var stream = new System.IO.MemoryStream((int)entry.UncompressedSize);

			//entry.Extract(stream);
			//stream.Position = 0;
			return new System.IO.BinaryReader(entry.OpenReader());
#endif
		}
