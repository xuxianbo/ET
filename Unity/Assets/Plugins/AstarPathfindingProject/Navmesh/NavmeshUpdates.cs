...
namespace Pathfinding {
	...
	public class NavmeshUpdates {
		...
		/// <summary>
		/// reset tile handle first can avoid huge update tile
		/// </summary>
		public static void ResetTileHandler(string name)
		{
			var graphs = AstarPath.active.graphs;
			for (int i = 0; i < graphs.Length; i++)
			{
				var navmeshBase = graphs[i] as NavmeshBase;
				if (navmeshBase == null || navmeshBase.name != name) continue;
				navmeshBase.navmeshUpdateData.Refresh(true);
