...
namespace Pathfinding {
	...
	public class BBTree : IAstarPooledObject {
		...
		public void RebuildFrom (TriangleMeshNode[] nodes) {
			...
			EnsureCapacity(Mathf.CeilToInt(length * 2.1f));
			// We will use approximately N node references
			EnsureNodeCapacity(Mathf.CeilToInt(length * 1.1f));
			...
			// we do not modify it
			var permutation = ArrayPool<int>.Claim(length);
			...
			var nodeBounds = ArrayPool<IntRect>.Claim(length);
			for (int i = 0; i < length; i++) {
				Int3 v0, v1, v2;
			...
			RebuildFromInternal(nodes, permutation, nodeBounds, 0, length, false);
