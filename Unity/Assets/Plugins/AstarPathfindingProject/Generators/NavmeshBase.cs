...
namespace Pathfinding {
	...
	public abstract class NavmeshBase : NavGraph, INavmesh, INavmeshHolder, ITransformedGraph
		...
		public override void GetNodes (System.Action<GraphNode> action) {
			...
			int tileXCount = this.tileXCount, tileLength = tiles.Length;
			for (int i = 0; i < tileLength; i++)
			{
				var tile = tiles[i];
				if (tile == null || tile.x + tile.z * tileXCount != i) continue;
				TriangleMeshNode[] nodes = tile.nodes;
				if (nodes == null) continue;
				int nodeLength = nodes.Length;
				for (int j = 0; j < nodeLength; j++) action(nodes[j]);
			}
		}
		...
		public void EndBatchTileUpdate () {
			...
			batchNodesToDestroy.Clear();

			var tiles = this.tiles;
			var batchUpdatedTiles = this.batchUpdatedTiles;
			int batchUpdatedTileCount = batchUpdatedTiles.Count;
			for (int i = 0; i < batchUpdatedTileCount; i++) tiles[batchUpdatedTiles[i]].flag = true;

			for (int i = 0; i < batchUpdatedTileCount; i++)
			{
				int batchUpdatedTile = batchUpdatedTiles[i];
				int x = batchUpdatedTile % tileXCount, z = batchUpdatedTile / tileXCount;
				if (x > 0) TryConnect(batchUpdatedTile, batchUpdatedTile - 1);
				if (x < tileXCount - 1) TryConnect(batchUpdatedTile, batchUpdatedTile + 1);
				if (z > 0) TryConnect(batchUpdatedTile, batchUpdatedTile - tileXCount);
				if (z < tileZCount - 1) TryConnect(batchUpdatedTile, batchUpdatedTile + tileXCount);
			}

			for (int i = 0; i < batchUpdatedTileCount; i++) tiles[batchUpdatedTiles[i]].flag = false;
		...
		protected override void DeserializeExtraInfo (GraphSerializationContext ctx) {
			...
			var tileXCount = reader.ReadInt32();
			this.tileXCount = tileXCount;
			if (tileXCount < 0) return;

			var tileZCount = reader.ReadInt32();
			this.tileZCount = tileZCount;
			transform = CalculateTransform();

			var tiles = new NavmeshTile[tileXCount * tileZCount];
			this.tiles = tiles;

			//Make sure mesh nodes can reference this graph
			TriangleMeshNode.SetNavmeshHolder((int)ctx.graphIndex, this);

			if(ByteArrayKClass == System.IntPtr.Zero) ByteArrayKClass = GetKClass(System.Array.Empty<byte>());
			var emptyNodes = System.Array.Empty<TriangleMeshNode>();
			var emptyInt3s = System.Array.Empty<Int3>();
			var emptyInts = System.Array.Empty<int>();

			for (int z = 0; z < tileZCount; z++) 
			{
				int zTileXCount = z * tileXCount;
				for (int x = 0; x < tileXCount; x++)
				{
					int tileIndex = x + zTileXCount;
					...
					if (trisCount > 0)
					{
						var tris = tile.tris = new int[trisCount];
						if (!Read(tris, trisCount, reader))
					...
					if (vertsCount > 0)
					{
						var verts = tile.verts = new Int3[vertsCount];
						if (!Read(verts, vertsCount, reader))
						{
							for (int i = 0; i < vertsCount; i++)
							{
								ref Int3 v = ref verts[i];
					...
					{
						tile.verts = emptyInt3s;
					}

					if (ctx.meta.version.Major >= 4) {
						int vertsInGraphSpaceCount = reader.ReadInt32();
						if (vertsInGraphSpaceCount != vertsCount) throw new System.Exception("Corrupt data. Array lengths did not match");

						if (vertsInGraphSpaceCount > 0)
						{
							tile.vertsInGraphSpace = new Int3[vertsInGraphSpaceCount];
							if (!Read(tile.vertsInGraphSpace, vertsInGraphSpaceCount, reader))
							{
								for (int i = 0; i < vertsInGraphSpaceCount; i++)
								{
									ref Int3 v = ref tile.vertsInGraphSpace[i];
									v.x = reader.ReadInt32();
									v.y = reader.ReadInt32();
									v.z = reader.ReadInt32();
								}
						...
						{
							tile.vertsInGraphSpace = emptyInt3s;
						}
					} else {
						// Compatibility
						if (vertsCount > 0)
						{
							tile.vertsInGraphSpace = new Int3[vertsCount];
					...
					if (nodeCount > 0)
					{
						var nodes = tile.nodes = new TriangleMeshNode[nodeCount];
						...
						for (int i = 0; i < nodeCount; i++)
						{
							...
							//序列化已经为v0,v1,v2赋值，以下内容不再需要 也可以考虑放弃序列化(需修改TriangleMeshNode内容并且重新导出序列化数据)，采用以下方式赋值，可以减少序列化数据大小
							//(int o0, int o1, int o2) = (node.v0, node.v1, node.v2);
							//node.v0 = tile.tris[i * 3 + 0] | tileIndex;
							//node.v1 = tile.tris[i * 3 + 1] | tileIndex;
							//node.v2 = tile.tris[i * 3 + 2] | tileIndex;

							//if (o0 != node.v0 || o1 != node.v1 || o2 != node.v2)
							//{
					...
					{
						tile.bbTree.RebuildFrom(tile.nodes = emptyNodes);
					}
				}
			}
		}
		...
		/// 读取数据
		/// </summary>
		protected unsafe bool Read<T>(T[] array, int length, BinaryReader reader) where T : unmanaged
		{
			if (!System.BitConverter.IsLittleEndian) return false;
			System.IntPtr kClass = System.IntPtr.Zero;
			ulong gcHandle = 0;
			System.IntPtr* address = default;
			try
			{
				byte[] data = System.Runtime.CompilerServices.Unsafe.As<byte[]>(array);
				int dataLength = length * sizeof(T);

				byte* ptr = (byte*)System.Runtime.CompilerServices.Unsafe.AsPointer(ref array[0]);
				var lengthPtr = (System.IntPtr*)(ptr - System.IntPtr.Size);

				//modify length
				*lengthPtr = (System.IntPtr)dataLength;
				//modify type
				address = (System.IntPtr*)Unity.Collections.LowLevel.Unsafe.UnsafeUtility.PinGCObjectAndGetAddress(data, out gcHandle);
				kClass = *address;
				*address = ByteArrayKClass;

				//read
				reader.Read(data, 0, dataLength);

				//revert length
				*lengthPtr = (System.IntPtr)length;
				//revert type
				*address = kClass;

				Unity.Collections.LowLevel.Unsafe.UnsafeUtility.ReleaseGCObject(gcHandle);

				return true;
			}
			catch
			{
				if (kClass != System.IntPtr.Zero)
				{
					*address = kClass;
					Unity.Collections.LowLevel.Unsafe.UnsafeUtility.ReleaseGCObject(gcHandle);
				}
		...
		/// 对象缓存
		/// </summary>
		static System.IntPtr ByteArrayKClass = System.IntPtr.Zero;
		/// <summary>
		/// 获取对象
		/// </summary>
		protected unsafe System.IntPtr GetKClass(object o)
		{
			void* address = Unity.Collections.LowLevel.Unsafe.UnsafeUtility.PinGCObjectAndGetAddress(o, out var gchandle);
			try
			{
				return *(System.IntPtr*)address;
			}
			finally
			{
				Unity.Collections.LowLevel.Unsafe.UnsafeUtility.ReleaseGCObject(gchandle);
